...
namespace DamageNumbersPro
{
    ...
    public class DamageNumberEditor : Editor
    {
        ...
        public static void PrepareWorldText(GameObject go)
        {
        ...
        public static void PrepareUIText(GameObject go)
        {
            if (!go.GetComponent<RectTransform>()) return;
            ...
            if (go.transform.Find("TextA") == null)
            {
                NewTextMeshUGUI("TextA", go.transform);
            }

            if (go.transform.Find("TextB") == null)
            {
                NewTextMeshUGUI("TextB", go.transform);
            }

            Undo.RegisterCreatedObjectUndo(go, "Create new Damage Number");
        }

        public static GameObject NewTextMeshUGUI(string tmName, Transform parent)
        {
            GameObject newTM = new GameObject();
            newTM.name = tmName;
        ...
        [MenuItem("GameObject/2D Object/Damage Number UI", priority = -1)]
        public static void CreateDamageNumberUI()
        {
            GameObject parentGO = Selection.activeGameObject;
            GameObject newDN = new GameObject("Damage Number UI");
            newDN.AddComponent<DamageNumber>();

            if (parentGO && parentGO.GetComponent<RectTransform>())
            {
                newDN.SetParentAndAlign(parentGO);
            }
            else
            {
                newDN.SetParentAndAlign(VT.Utilities.Factory.VTObjectFactory.CreateSceneCanvas());
            }
        ...
        [MenuItem("GameObject/2D Object/Damage Number World",priority = -1)]
        public static void CreateDamageNumberWorld()
        {
            GameObject newDN = new GameObject("Damage Number World");
            newDN.AddComponent<DamageNumber>();
            PrepareWorldText(newDN);
        ...
        public override void OnInspectorGUI()
        {
            ...
            {
                if (GUILayout.Button("Prepare World Text"))
                {
                    PrepareWorldText(dn.gameObject);
                    OnEnable();
                }
                #region VT Framework Extension
                else if (GUILayout.Button("Prepare UI Text"))
                {
                    PrepareUIText(dn.gameObject);
                    OnEnable();
                }
                #endregion
