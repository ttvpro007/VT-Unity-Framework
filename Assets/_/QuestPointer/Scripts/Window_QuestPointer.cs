...
public class Window_QuestPointer : MonoBehaviour {
    ...
    private static Window_QuestPointer instance;
    public static Window_QuestPointer GetInstance() => instance;

    public static QuestPointer Create(Vector3 targetPosition, Color arrowColor, Color crossColor, Sprite arrowSprite = null, Sprite crossSprite = null) {
        return instance.CreatePointer(targetPosition, arrowColor, crossColor, arrowSprite, crossSprite);
    }

    public static void DestroyPointer(Vector3 pointerPosition, float maxDistance = 1f) {
        for (int i = 0; i < instance.questPointerList.Count; i++) {
            if (Vector3.Distance(instance.questPointerList[i].GetTargetPosition(), pointerPosition) <= maxDistance) {
                instance.DestroyPointer(instance.questPointerList[i]);
                i--;
            }
        }
    }
