...
#region VT Framework Extension
//using BuilderDefender.GameDataManagement;
#endregion
...
#region Perspective
        ...
        public void GetReferences()
        {
            ...
            textA = transform.Find("TextA").GetComponent<TMP_Text>();
            textB = transform.Find("TextB").GetComponent<TMP_Text>();
        }
        ...
        public TMP_Text GetTextA()
        {
            return textA;
        }
        public TMP_Text GetTextB()
        {
            return textB;
        }
...
#region VT Framework Extension
        ...
        public DamageNumber CreateNew(string textValue, Vector3 newPosition)
        {
            DamageNumber newDN = default;
            int instanceID = GetInstanceID();
        ...
        public static DamageNumber CreateNew(GameObject damageNumberPrefab, float numberValue, Vector3 spawnPosition)
        {
            DamageNumber damageNumber = damageNumberPrefab.GetComponent<DamageNumber>();
            if (damageNumber != null)
            {
                damageNumber.enableNumber = true;
                return damageNumber.CreateNew(numberValue, spawnPosition);
            }
            return null;
        }

        public static DamageNumber CreateNew(GameObject damageNumberPrefab, string textValue, Vector3 spawnPosition)
        {
            DamageNumber damageNumber = damageNumberPrefab.GetComponent<DamageNumber>();
            if (damageNumber != null)
            {
                damageNumber.enableNumber = false;
                return damageNumber.CreateNew(textValue, spawnPosition);
            }
            return null;
        }

        private static DNPPrefabsLibrarySO dnpPrefabsLibrarySO;
        public static DamageNumber CreateNew(Enums.DNPPrefabs dnpPrefabs, float numberValue, Vector3 spawnPosition)
        {
            if (!dnpPrefabsLibrarySO)
            {
                dnpPrefabsLibrarySO = Resources.Load<DNPPrefabsLibrarySO>(typeof(DNPPrefabsLibrarySO).Name);
            }

            if (dnpPrefabsLibrarySO)
            {
                GameObject dnpPrefab = dnpPrefabsLibrarySO.DNPPrefabDetails[dnpPrefabs.ToString()];
                if (dnpPrefab)
                {
                    return CreateNew(dnpPrefab, numberValue, spawnPosition);
                }
            }
